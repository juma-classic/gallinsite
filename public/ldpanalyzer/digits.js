...
function processTick(price, isHistorical = false) {
    try {
        ...
        const parts = priceStr.split('.');
        const lastDigit = parts[1] && parts[1].length >= 3 ? parseInt(parts[1][2]) : 0; // Changed from [1] to [2] for 3rd decimal
...
function requestCandles() {
    ...
    console.log(
        `📈 Requesting ${candleCount} historical candles (${candlesPerHour}/hour) for ${state.activeSubscription}`
    );
}
...
function exportData() {
    try {
        ...
        if (state.last1000Ticks.length > 0) {
            ...
            data.statistics = {
                digitCounts,
                percentages: digitCounts.map(count => ((count / state.last1000Ticks.length) * 100).toFixed(2)),
...
function getStatistics() {
    ...
    const hotDigits = counts
        .map((count, index) => ({ digit: index, count }))
        .filter(item => item.count === maxCount)
        .map(item => item.digit);

    const coldDigits = counts
        .map((count, index) => ({ digit: index, count }))
...
function initThemeSwitch() {
    ...
    const isDexteratorPage =
        currentPage.includes('index.html') || currentPage.endsWith('/ai/') || currentPage.endsWith('/ai');
    ...
    if (themeSwitch) {
        themeSwitch.addEventListener('click', () => {
            ...
            console.log(
                `🎨 Theme switched - Page: ${isRoyalPage ? 'Royal' : isMinimalistPage ? 'Minimalist' : 'Dexterator'}`
            );
        });
    }
}
